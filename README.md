...
- **Training steps in Stage 2**: If you change the batch sizes when fine-tuning ATST-Frame (Stage 2), you might probably need to change the `n_epochs` in the configuration file `train/local/confs/stage2.yaml` correspondingly. According to our experiments, the model converge at 5.8k - 7.2k steps. Extra training steps might not help the model to perform better. E.g., if you change the batch size to [4, 4, 8, 8], the `n_epochs` could be changed to `40` ~ `50` correspondingly. 
